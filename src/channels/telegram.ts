// ---------------------------------------------------------------------------
// OpenWebClaw — Telegram Bot API Channel
// ---------------------------------------------------------------------------
  ...
  async send(groupId: string, text: string): Promise<void> {
    ...
    for (const chunk of chunks) {
      try {
        ...
        console.warn('Failed to send via Telegram with Markdown. Retrying as plain text...', err);
        await this.apiCall('sendMessage', {
          chat_id: chatId,
          text: chunk,
        }).catch(fallbackErr => {
          console.error('Telegram plain text fallback also failed:', fallbackErr);
        });
      }
    }
  }
  ...
  private handleUpdate(update: TelegramUpdate): void {
    ...
    if (msg.text === '/chatid') {
      this.apiCall('sendMessage', {
        chat_id: chatId,
        text: `Chat ID: \`${chatId}\`\nRegister this ID in OpenWebClaw settings.`,
        parse_mode: 'Markdown',
      }).catch(console.error);
      return;
    }
    ...
    if (msg.text === '/ping') {
      this.apiCall('sendMessage', {
        chat_id: chatId,
        text: 'Pong! 🏓 OpenWebClaw is running.',
      }).catch(console.error);
      return;
    }
    ...
    if (msg.text === '/debug') {
      const isRegistered = this.registeredChatIds.has(chatId);
      this.apiCall('sendMessage', {
        chat_id: chatId,
        text: `🔧 *Debug:*
Chat ID: \`${chatId}\`
Registered: ${isRegistered ? '✅' : '❌'}
Whitelist: \`${Array.from(this.registeredChatIds).join(', ') || '(empty)'}\``,
        parse_mode: 'Markdown',
      }).catch(console.error);
      return;
    }
